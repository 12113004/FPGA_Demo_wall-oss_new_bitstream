...
void HBM_mvm_test(struct FPGA_HBM_MVM_cfg cfg, char* path_name, char* head_name, struct bin_inf* weight_in_bin_inf, struct bin_inf* scales_in_bin_inf, struct bin_inf* dat_in_bin_inf, 
                struct bin_inf* wt_and_scale_in_HBM_inf[], int en_0, struct bin_inf* dat_in_HBM_inf[], int en_1)
{
    printf("MVM Test \n");
    ...
    if(en_0) WT_AND_SCALE_TRANS_FUNCTION_MVM(cfg, wt, wt_FP_scale, wt_and_scale_in_HBM);
    if(en_1) DAT_IN_TRANS_FUNCTION_MVM(cfg, dat_in, dat_in_HBM);

    uint64_t HBM_base_address =0;
    char* filepath = (char*)malloc(sizeof(char)*200);
    sprintf(filepath, "./%s/%s", path_name, "MVM_HBM_bin");
    if(en_0)
    {
...
void HBM_mvm_receive_and_compare(struct FPGA_HBM_MVM_cfg cfg, HANDLE c2hx_device, char* path_name, char* head_name, struct bin_inf* standard_out_bin_inf)
{
    ...
    sprintf(filepath1, "./%s/%s", path_name, "MVM_FPGA_out_bin/HBM_bin");
    for(uint64_t m=0;m<group;m++)
    {
        char* write_filename1 = (char*)malloc(sizeof(char)*200);
    ...
    /***** Detransform*****/
    DAT_OUT_TRANS_FUNCTION_MVM(cfg, dat_out_HBM, dat_out);
    ...
    sprintf(filepath2, "./%s/%s", path_name, "MVM_FPGA_out_bin");
