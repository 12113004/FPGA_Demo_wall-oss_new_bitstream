...
void HBM_mvmbn_wt_test(struct FPGA_HBM_MVM_BN_cfg cfg, char* path_name, char* head_name, struct bin_inf* weight_in_bin_inf, struct bin_inf* scales_in_bin_inf, struct bin_inf* wt_and_scale_in_HBM_inf[], int en)
{
    printf("MVM_BN_Argmax data Test \n");
    ...
    if(en) WT_AND_SCALE_TRANS_FUNCTION_MVM_BN(cfg, wt, wt_FP_scale, wt_and_scale_in_HBM);
...
void HBM_mvmbn_data_and_bn_test(struct FPGA_HBM_MVM_BN_cfg cfg, char* path_name, char* head_name, struct bin_inf* dat_in_bin_inf, struct bin_inf* bn_wt_in_bin_inf, struct bin_inf* bn_bias_in_bin_inf, 
                  struct bin_inf* dat_in_HBM_inf[], int en_0, struct bin_inf* bn_wt_and_bias_in_HBM_inf[], int en_1)
{
    printf("MVM_BN_Argmax dat and bn Test \n");
    ...
    if(en_0) DAT_IN_TRANS_FUNCTION_MVM_BN(cfg, dat_in, dat_in_HBM);
    if(en_1) BN_WT_AND_BIAS_IN_TRANS_FUNCTION_MVM_BN(cfg, bn_wt, bn_bias, bn_wt_and_bias_in_HBM);
...
void HBM_mvmbn_argmax_receive_and_find_ID(struct FPGA_HBM_MVM_BN_cfg cfg, HANDLE c2hx_device, char* path_name, char* head_name)
{
    ...
    sprintf(filepath1, "./%s/%s", path_name, "MVM_BN_Argmax_FPGA_out_bin/HBM_bin");
    for(uint64_t m=0;m<group;m++)
    {
        char* write_filename1 = (char*)malloc(sizeof(char)*200);
    ...
    sprintf(filepath2, "./%s/%s", path_name, "MVM_BN_Argmax_FPGA_out_bin");
    ...
    /***** Compare Bin *****/
    printf_fp16_log(dat_out_demaped_inf->bin_data_file, dat_out_demaped_inf->bin_data_size, "argmax.log");
    
    half *argmax_data_half = (half*)malloc(sizeof(half)*cfg.Hout*cfg.Wout*cfg.CHout);
    if (argmax_data_half == NULL){printf("fail to malloc argmax_data_half \n");}

    uint16_to_half(dat_out[0], cfg.Hout*cfg.Wout*cfg.CHout, argmax_data_half);

    int max_Id = 0;
    half max_value = argmax_data_half[0];

    for (int i=1;i<cfg.Hout*cfg.Wout*cfg.CHout;i++) 
    {
    ...
    free(argmax_data_half);
    argmax_data_half = NULL;
}