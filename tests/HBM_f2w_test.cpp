...
void HBM_f2w_test(struct FPGA_HBM_F2W_cfg cfg, char* path_name, char* head_name, struct bin_inf* dat_in_bin_inf, struct bin_inf* wt_in_bin_inf, struct bin_inf* *dat_in_HBM_inf, int en_0, struct bin_inf* *wt_in_HBM_inf, int en_1)
{
    printf("F2W Test \n");
    ...
    /***** Transform *****/
    if(en_0) DAT_IN_TRANS_FUNCTION_F2W(cfg, dat_in, dat_in_HBM);
    if(en_1) WT_IN_TRANS_FUNCTION_F2W(cfg, wt_in, wt_in_HBM);

    /***** Generate Bin*****/
    uint64_t HBM_base_address =0;
    char* filepath = (char*)malloc(sizeof(char)*200);
    sprintf(filepath, "./%s/%s", path_name, "F2W_HBM_bin");
    if(en_0)
    {
...
void HBM_f2w_receive_and_compare(struct FPGA_HBM_F2W_cfg cfg, HANDLE c2hx_device, char* path_name, char* head_name, struct bin_inf* standard_out_bin_inf)
{
    /***** Generate Bin and Receive Data *****/
    struct bin_inf* *dat_out_HBM_inf = (struct bin_inf**)malloc(sizeof(struct bin_inf)*group);
    int dat_out_HBM_size = cfg.Weight_Head*cfg.Wout*cfg.Hout*cfg.Head_x_CHout_div_LTout*MAX_DAT_DW*L_Tout/32/group;
    uint64_t HBM_base_address =0;
    char* filepath1 = (char*)malloc(sizeof(char)*200);
    sprintf(filepath1, "./%s/%s", path_name, "F2W_FPGA_out_bin/HBM_bin");
    ...
    /***** Detransform*****/
    DAT_OUT_TRANS_FUNCTION_F2W(cfg, dat_out_HBM, dat_out);
    ...
    sprintf(filepath2, "./%s/%s", path_name, "F2W_FPGA_out_bin");
