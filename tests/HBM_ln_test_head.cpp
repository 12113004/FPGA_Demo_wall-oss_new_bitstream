...
void HBM_ln_test(struct FPGA_HBM_LN_cfg cfg, char* path_name, char* head_name, struct bin_inf* dat_in_bin_inf, struct bin_inf* ln_weight_bin_inf, struct bin_inf* ln_bias_bin_inf, struct bin_inf* dat_in_HBM_inf[], int en_0, struct bin_inf* ln_wt_and_bias_HBM_inf[], int en_1)
{
    printf("LN Test \n");
    ...
    int *LN_weight = (int*)malloc(sizeof(int)*cfg.CHout);
    ...
    int *LN_bias = (int*)malloc(sizeof(int)*cfg.CHout);
    ...
    int LN_wt_and_bias_HBM_size = cfg.LN_ch_group_times*MAX_DAT_DW*L_Tout/32/group;
    for(int i=0;i<group;i++)
    {
    ...
    /***** Transform *****/
    if(en_0) DAT_IN_TRANS_FUNCTION_LN(cfg, dat_in, dat_in_HBM);
    if(en_1) LN_WEIGHT_AND_BIAS_IN_TRANS_FUNCTION_LN(cfg, LN_weight, LN_bias, LN_wt_and_bias_HBM);
...
void HBM_ln_receive_and_compare(struct FPGA_HBM_LN_cfg cfg, HANDLE c2hx_device, char* path_name, char* head_name, struct bin_inf* standard_out_bin_inf)
{
    ...
    /***** Detransform*****/
    DAT_OUT_TRANS_FUNCTION_LN(cfg, dat_out_HBM, dat_out);
