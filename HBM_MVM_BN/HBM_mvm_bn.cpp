...
#include "General_Map_mvm_bn.h"

struct FPGA_HBM_MVM_BN_cfg GetFPGA_HBM_MVM_BN_cfg(int Height, int Hin, int Width_in, int Width_out, uint64_t DAT_IN_BASE_ADDR, uint64_t HBM00_WT_BASE_ADDR, uint64_t BN_BASE_ADDR, uint64_t DAT_OUT_BASE_ADDR)
{
    struct FPGA_HBM_MVM_BN_cfg ret;
    ...
    ret.BN_CH_per_AXI_DW              = (MAX_DAT_DW*L_Tout/(2*ret.BN_DW));
    ret.BN_ch_group_times             = (ret.CHout_Padding/ret.BN_CH_per_AXI_DW);
    ret.BN_SIZE_IN_BYTES              = (ret.BN_DW*2*2*ret.CHout_Padding_with_Tout);
    ret.BN_WT_scale                   = 0;
    ret.BN_BIAS_scale                 = 0;
...
void WT_AND_SCALE_TRANS_FUNCTION_MVM_BN(struct FPGA_HBM_MVM_BN_cfg cfg , int *wt, int *wt_FP_scale, int *HBM[])
{
...
void DAT_IN_TRANS_FUNCTION_MVM_BN(struct FPGA_HBM_MVM_BN_cfg cfg, int *dat_in[], int *HBM[])
{
...
void BN_WT_AND_BIAS_IN_TRANS_FUNCTION_MVM_BN(struct FPGA_HBM_MVM_BN_cfg cfg, int *bn_wt, int *bn_bias, int *HBM[])
{
    // [`MAX_BN_DW*2-1:0] bn_wt_and_bias [`CHout];
    int *bn_wt_and_bias_in = (int*)malloc(sizeof(int)*cfg.CHout);
    if (bn_wt_and_bias_in == NULL){printf("fail to malloc bn_wt_and_bias \n");}

    // [`MAX_DAT_DW*`L_Tout-1:0] bn_wt_and_bias_mem [2*`CHout_div_LTout];
    int **bn_wt_and_bias_in_mem = (int**)malloc(sizeof(int*)*2*cfg.CHout_div_LTout);
    for(int i=0;i<2*cfg.CHout_div_LTout;i++)
    {
        bn_wt_and_bias_in_mem[i] = (int*)malloc(sizeof(int)*MAX_DAT_DW*L_Tout/32);
    ...
    for(int i=0;i<cfg.BN_ch_group_times;i++)        // depth
    {
        for(int j=0;j<MAX_DAT_DW*L_Tout/32;j++)     // 8192bit/32bit = 256
        {
            if(j>=8* 0 & j<8* 1) HBM[ 0][i*HBM_AXI_DATA_WIDTH/32+(j-8* 0)] = bn_wt_and_bias_in_mem[i][j];
            if(j>=8* 1 & j<8* 2) HBM[ 1][i*HBM_AXI_DATA_WIDTH/32+(j-8* 1)] = bn_wt_and_bias_in_mem[i][j];
            if(j>=8* 2 & j<8* 3) HBM[ 2][i*HBM_AXI_DATA_WIDTH/32+(j-8* 2)] = bn_wt_and_bias_in_mem[i][j];
            if(j>=8* 3 & j<8* 4) HBM[ 3][i*HBM_AXI_DATA_WIDTH/32+(j-8* 3)] = bn_wt_and_bias_in_mem[i][j];
            if(j>=8* 4 & j<8* 5) HBM[ 4][i*HBM_AXI_DATA_WIDTH/32+(j-8* 4)] = bn_wt_and_bias_in_mem[i][j];
            if(j>=8* 5 & j<8* 6) HBM[ 5][i*HBM_AXI_DATA_WIDTH/32+(j-8* 5)] = bn_wt_and_bias_in_mem[i][j];
            if(j>=8* 6 & j<8* 7) HBM[ 6][i*HBM_AXI_DATA_WIDTH/32+(j-8* 6)] = bn_wt_and_bias_in_mem[i][j];
            if(j>=8* 7 & j<8* 8) HBM[ 7][i*HBM_AXI_DATA_WIDTH/32+(j-8* 7)] = bn_wt_and_bias_in_mem[i][j];
            if(j>=8* 8 & j<8* 9) HBM[ 8][i*HBM_AXI_DATA_WIDTH/32+(j-8* 8)] = bn_wt_and_bias_in_mem[i][j];
            if(j>=8* 9 & j<8*10) HBM[ 9][i*HBM_AXI_DATA_WIDTH/32+(j-8* 9)] = bn_wt_and_bias_in_mem[i][j];
            if(j>=8*10 & j<8*11) HBM[10][i*HBM_AXI_DATA_WIDTH/32+(j-8*10)] = bn_wt_and_bias_in_mem[i][j];
            if(j>=8*11 & j<8*12) HBM[11][i*HBM_AXI_DATA_WIDTH/32+(j-8*11)] = bn_wt_and_bias_in_mem[i][j];
            if(j>=8*12 & j<8*13) HBM[12][i*HBM_AXI_DATA_WIDTH/32+(j-8*12)] = bn_wt_and_bias_in_mem[i][j];
            if(j>=8*13 & j<8*14) HBM[13][i*HBM_AXI_DATA_WIDTH/32+(j-8*13)] = bn_wt_and_bias_in_mem[i][j];
            if(j>=8*14 & j<8*15) HBM[14][i*HBM_AXI_DATA_WIDTH/32+(j-8*14)] = bn_wt_and_bias_in_mem[i][j];
            if(j>=8*15 & j<8*16) HBM[15][i*HBM_AXI_DATA_WIDTH/32+(j-8*15)] = bn_wt_and_bias_in_mem[i][j];
            if(j>=8*16 & j<8*17) HBM[16][i*HBM_AXI_DATA_WIDTH/32+(j-8*16)] = bn_wt_and_bias_in_mem[i][j];
            if(j>=8*17 & j<8*18) HBM[17][i*HBM_AXI_DATA_WIDTH/32+(j-8*17)] = bn_wt_and_bias_in_mem[i][j];
            if(j>=8*18 & j<8*19) HBM[18][i*HBM_AXI_DATA_WIDTH/32+(j-8*18)] = bn_wt_and_bias_in_mem[i][j];
            if(j>=8*19 & j<8*20) HBM[19][i*HBM_AXI_DATA_WIDTH/32+(j-8*19)] = bn_wt_and_bias_in_mem[i][j];
            if(j>=8*20 & j<8*21) HBM[20][i*HBM_AXI_DATA_WIDTH/32+(j-8*20)] = bn_wt_and_bias_in_mem[i][j];
            if(j>=8*21 & j<8*22) HBM[21][i*HBM_AXI_DATA_WIDTH/32+(j-8*21)] = bn_wt_and_bias_in_mem[i][j];
            if(j>=8*22 & j<8*23) HBM[22][i*HBM_AXI_DATA_WIDTH/32+(j-8*22)] = bn_wt_and_bias_in_mem[i][j];
            if(j>=8*23 & j<8*24) HBM[23][i*HBM_AXI_DATA_WIDTH/32+(j-8*23)] = bn_wt_and_bias_in_mem[i][j];
            if(j>=8*24 & j<8*25) HBM[24][i*HBM_AXI_DATA_WIDTH/32+(j-8*24)] = bn_wt_and_bias_in_mem[i][j];
            if(j>=8*25 & j<8*26) HBM[25][i*HBM_AXI_DATA_WIDTH/32+(j-8*25)] = bn_wt_and_bias_in_mem[i][j];
            if(j>=8*26 & j<8*27) HBM[26][i*HBM_AXI_DATA_WIDTH/32+(j-8*26)] = bn_wt_and_bias_in_mem[i][j];
            if(j>=8*27 & j<8*28) HBM[27][i*HBM_AXI_DATA_WIDTH/32+(j-8*27)] = bn_wt_and_bias_in_mem[i][j];
            if(j>=8*28 & j<8*29) HBM[28][i*HBM_AXI_DATA_WIDTH/32+(j-8*28)] = bn_wt_and_bias_in_mem[i][j];
            if(j>=8*29 & j<8*30) HBM[29][i*HBM_AXI_DATA_WIDTH/32+(j-8*29)] = bn_wt_and_bias_in_mem[i][j];
            if(j>=8*30 & j<8*31) HBM[30][i*HBM_AXI_DATA_WIDTH/32+(j-8*30)] = bn_wt_and_bias_in_mem[i][j];
            if(j>=8*31 & j<8*32) HBM[31][i*HBM_AXI_DATA_WIDTH/32+(j-8*31)] = bn_wt_and_bias_in_mem[i][j];  
        }
    }

    // Free malloc
    free(bn_wt_and_bias_in);
    bn_wt_and_bias_in = NULL;
    free(bn_wt_and_bias_in_mem);
    bn_wt_and_bias_in_mem = NULL;
}

void DAT_OUT_TRANS_FUNCTION_MVM_BN(struct FPGA_HBM_MVM_BN_cfg cfg, int *HBM[], uint16_t *dat_out[])
{
