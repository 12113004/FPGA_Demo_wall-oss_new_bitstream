...
struct FPGA_HBM_MVM_BN_cfg GetFPGA_HBM_MVM_BN_cfg(int Height, int Win, int Width_in, int Width_out, int Skip_Factor, int RELU_EN, int log2_WT_base_addr_Bank_Step, 
                                                  uint64_t DAT_IN_BASE_ADDR, uint64_t HBM00_WT_BASE_ADDR, uint64_t BN_BASE_ADDR, uint64_t DAT_OUT_BASE_ADDR)
{
...
void WT_TRANS_FUNCTION_MVM_BN(struct FPGA_HBM_MVM_BN_cfg cfg , int *wt, int *wt_FP_scale, int *HBM_DDR[])
{
                   ...
                   for(int n=0;n<cfg.WT_CH_Tgroup_div_Tblock*16/32;n++)
                   {
                        ...
                        for(int p=0; p<2; p++)
                        {
                            if((i*Tout+k*HBM_Port+m<cfg.CHout) && (j*cfg.WT_CH_Tgroup_div_Tblock+n*2+p<cfg.WT_CHin_div_Tblock))
                            {
                               ...
                               //if(i==0 && j==0 && k==0 && m==1){
                               //    printf("wt_scale[%d] = %x \n", i*cfg.WT_CHin_div_Tblock*Tout + (j*cfg.WT_CH_Tgroup_div_Tblock+n*2+p) + (k*HBM_Port+m)*cfg.WT_CHin_div_Tblock, wt_FP_scale[i*cfg.WT_CHin_div_Tblock*Tout + (j*cfg.WT_CH_Tgroup_div_Tblock+n*2+p) + (k*HBM_Port+m)*cfg.WT_CHin_div_Tblock]);
                            }
                        }
...
//					{ std::cout << "***scale_addr_bias="<<scale_addr_bias<<std::endl;}
					for(int n=0;n<HBM_AXI_DATA_WIDTH/32;n++)
                    {    
//                        if(m == 1)
//                            {printf("scale_addr_bias + n: %d.\n\r", (i*cfg.WT_scale_group_nums*Tout/HBM_Port*HBM_Port + j*Tout/HBM_Port*HBM_Port + k*HBM_Port + m)*HBM_AXI_DATA_WIDTH/32+n);}
                        ...
                        //{  std::cout << "addr=" << scale_addr_bias + n << ", scale_HBM_DDR[CHout_div_Tout" << i << "][Group" << j << "][Tout_div_Port" << k << "][Port" << m << "][CHin" << n << "]=" << (HBM_wt_FP_scale[i*cfg.WT_scale_group_nums*Tout/HBM_Port*HBM_Port + j*Tout/HBM_Port*HBM_Port + k*HBM_Port + m]>> (32 * n)& mask) << std::endl; }
                    }
                }
            }
        }
    		...
    		for(int k=0;k<Tout/HBM_Port;k++)
            {
    			for(int m=0;m<HBM_Port;m++)
                {
                    ...
                    wt_addr_bias=static_cast<int>((i*cfg.CHin_WT_and_Scale_Bytes+j*cfg.Group_WT_and_Scale_Bytes)*8/32*(Tout/HBM_Port)+cfg.Group_Scale_Bytes*8/32 
                                                   + ((j==cfg.WT_scale_group_nums-1)? (k*cfg.Last_Group_WT_and_Scale_Bytes*8/32) : (k*cfg.Group_WT_and_Scale_Bytes*8/32)));
                                                   //+cfg.HBM00_WT_BASE_ADDR/4+cfg.WT_base_addr_Bank_Step/4*m);
//					{ std::cout << "***wt_addr_bias="<<wt_addr_bias<<std::endl;}
//					{ std::cout << "***wt_addr_bias="<<wt_addr_bias<< "\t***wt_start_ch_in="<<wt_start_ch_in<<"\twt_end_ch_in"<<wt_end_ch_in<<std::endl;}
...
void DAT_IN_TRANS_FUNCTION_MVM_BN(struct FPGA_HBM_MVM_BN_cfg cfg, int *dat_in[], int *HBM_DDR[])
{
...
void BN_WT_AND_BIAS_TRANS_FUNCTION_MVM_BN(struct FPGA_HBM_MVM_BN_cfg cfg, int *bn_wt, int *bn_bias, int *HBM_DDR[])
{
...
void DAT_OUT_DEMAP_FUNCTION_MVM_BN(struct FPGA_HBM_MVM_BN_cfg cfg, uint16_t *dat_out_mem, uint16_t *dat_out[])
{
