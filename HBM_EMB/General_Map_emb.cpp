...
{
    for(int k=0;k<cfg.CHin_div_LTout;k=k+1)
    {
        for(int j=0;j<cfg.Pos_Num;j++)
        {
            int tmp [MAX_DAT_DW*L_Tout/32*2] = {0};
            for(int kk=0;kk<L_Tout;kk++)
            {
                if(k*L_Tout+kk<cfg.CHin) 
                   tmp[kk]=in[j*cfg.CHin+k*L_Tout+kk]&0x0000ffff;
                else
                   tmp[kk]=0;
            }
            for(int m=0;m<MAX_DAT_DW*L_Tout/32;m++)
                mem[cfg.Pos_Num*k+j][m]=(tmp[2*m+1]<<MAX_DAT_DW)+tmp[2*m];
        }
    }
}
