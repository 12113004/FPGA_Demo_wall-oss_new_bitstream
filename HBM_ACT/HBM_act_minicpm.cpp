...
#include "General_Map_act_minicpm.h"

struct FPGA_HBM_ACT_MINICPM_cfg GetFPGA_HBM_ACT_MINICPM_cfg(int Height, int Hin, int Width_in, uint64_t DAT_IN_BASE_ADDR, uint64_t WT_BASE_ADDR, uint64_t DAT_OUT_BASE_ADDR)
{
    struct FPGA_HBM_ACT_MINICPM_cfg ret;
...
void DAT_IN_TRANS_FUNCTION_ACT_MINICPM(struct FPGA_HBM_ACT_MINICPM_cfg cfg, int *dat_in[], int *HBM[])
{
...
void PARAMETER_IN_TRANS_FUNCTION_ACT_MINICPM(struct FPGA_HBM_ACT_MINICPM_cfg cfg, int *wt, int *bias, int* x_region, int *HBM[])
{
    // bit [`AXI_DAT_WIDTH-1:0] parameter_in_mem[3];
    int **parameter_in_mem = (int**)malloc(sizeof(int*)*3);
    for(int i=0;i<3;i++)
    {
        parameter_in_mem[i] = (int*)malloc(sizeof(int)*HBM_AXI_DATA_WIDTH/32);
        if (parameter_in_mem[i] == NULL){printf("fail to malloc parameter_in_mem \n");}
    }

    for(int i=0;i<HBM_AXI_DATA_WIDTH/32;i++)
    {
        parameter_in_mem[0][i] = ((wt      [2*i+1]<<16)&0xffff0000) + (wt      [2*i]&0x0000ffff);
        parameter_in_mem[1][i] = ((bias    [2*i+1]<<16)&0xffff0000) + (bias    [2*i]&0x0000ffff);
        parameter_in_mem[2][i] = ((x_region[2*i+1]<<16)&0xffff0000) + (x_region[2*i]&0x0000ffff);
    }
    
	for(int i=0;i<3;i++)
    {
		for(int j=0;j<HBM_AXI_DATA_WIDTH/32;j++)
            HBM[0][i*HBM_AXI_DATA_WIDTH/32+j] = parameter_in_mem[i][j];
    }

    // Free malloc
    free(parameter_in_mem);
    parameter_in_mem = NULL;
}

void DAT_OUT_TRANS_FUNCTION_ACT_MINICPM(struct FPGA_HBM_ACT_MINICPM_cfg cfg, int *HBM[], uint16_t *dat_out[])
{
