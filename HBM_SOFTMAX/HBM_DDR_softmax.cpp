...
struct FPGA_HBM_SOFTMAX_cfg GetFPGA_HBM_SOFTMAX_cfg(int Height, int Head, int Width_in, int DAT_IN_scale, uint64_t DAT_IN_BASE_ADDR, uint64_t DAT_OUT_BASE_ADDR)
{
    struct FPGA_HBM_SOFTMAX_cfg ret;

    ret.Height                   = Height;      //19
    ret.Head                     = Head;        //32
    ret.Width_in                 = Width_in;    //19
    ...
    ret.Win                      = ret.Height;  //(((Height+AXI_BURST_LEN_SOFTMAX-1)/AXI_BURST_LEN_SOFTMAX)*AXI_BURST_LEN_SOFTMAX);
    ret.Hin                      = ret.Head;
    ret.CHin                     = Width_in;
    ret.CHout                    = ret.CHin;
...
void DAT_IN_TRANS_FUNCTION_SOFTMAX(struct FPGA_HBM_SOFTMAX_cfg cfg, int *dat_in[], int *HBM_DDR[])
{
...
void DAT_OUT_DEMAP_FUNCTION_SOFTMAX(struct FPGA_HBM_SOFTMAX_cfg cfg, uint16_t *dat_out_mem, uint16_t *dat_out[])
{
