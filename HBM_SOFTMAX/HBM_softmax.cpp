...
{
    struct FPGA_HBM_SOFTMAX_cfg ret;
...
{
    // bit [`MAX_DAT_DW*`L_Tout-1:0] dat_in_mem [`Head*`Hin*`Win*`CHin_div_LTout];
    int **dat_in_mem = (int**)malloc(sizeof(int*)*cfg.Head*cfg.Hin*cfg.Win*cfg.CHin_div_LTout);
    for (int i=0;i<cfg.Head*cfg.Hin*cfg.Win*cfg.CHin_div_LTout;i++)
    {
        dat_in_mem[i] = (int*)malloc(sizeof(int)*MAX_DAT_DW*L_Tout/32);
        if(dat_in_mem[i] == NULL){printf("fail to malloc dat_in_mem \n");}
    }

    // bit [`MAX_DAT_DW*`L_Tout-1:0] tp_dat_in_mem[`Head][`Win*`Hin*`CHin_div_LTout];
    int ***tp_dat_in_mem = (int***)malloc(sizeof(int**)*cfg.Head);
    ...
    for(int i=0;i<cfg.Head*cfg.Win*cfg.Hin*cfg.CHin_div_LTout;i++)   // depth
    {
...
void DAT_OUT_TRANS_FUNCTION_SOFTMAX(struct FPGA_HBM_SOFTMAX_cfg cfg, int *HBM[], uint16_t *dat_out[])
{
    ...
    int **dat_out_mem_tmp = (int**)malloc(sizeof(int*)*cfg.Head*cfg.Hout*cfg.Wout*cfg.CHout_div_LTout);
    for (int i=0;i<cfg.Head*cfg.Hout*cfg.Wout*cfg.CHout_div_LTout;i++)
    {
    ...
    uint16_t **dat_out_mem = (uint16_t**)malloc(sizeof(uint16_t*)*cfg.Head*cfg.Hout*cfg.Wout*cfg.CHout_div_LTout);
    for (int i=0;i<cfg.Head*cfg.Hout*cfg.Wout*cfg.CHout_div_LTout;i++)
    {
    ...
    // bit [`MAX_DAT_DW*`L_Tout-1:0] tp_dat_out_mem [`Head ][`Hout* `Wout *`CHout_div_LTout];
    uint16_t ***tp_dat_out_mem = (uint16_t***)malloc(sizeof(uint16_t**)*cfg.Head);
    for(int i=0;i<cfg.Head;i++)
    {
    ...
    for(int i=0;i<cfg.Head*cfg.Wout*cfg.Hout*cfg.CHout_div_LTout;i++)
    {
        for(int j=0;j<MAX_DAT_DW*L_Tout/32;j++)
        {
