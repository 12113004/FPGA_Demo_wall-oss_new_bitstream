...
struct FPGA_HBM_LN_cfg GetFPGA_HBM_LN_cfg(int Height, int Hin, int Width_in, int Head, uint64_t DAT_IN_BASE_ADDR, uint64_t LN_WT_BASE_ADDR, uint64_t DAT_OUT_BASE_ADDR)
{
    ...
    ret.Head_x_CHin_div_LTout  = (ret.CHin>=L_Tout) ? ret.Head*ret.CHin_Padding : ret.Head*ret.CHin
    ...
    for(int b=0;b<Tb;b++) 
    {
        for(int i=0;i<cfg.Win*cfg.Hin*cfg.Head_x_CHin_div_LTout;i++)
        {
            for(int j=0;j<MAX_DAT_DW*L_Tout/32;j++)
                dat_in_mem[b*cfg.Win*cfg.Hin*cfg.Head_x_CHin_div_LTout+i][j] = tp_dat_in_mem[b][i][j];
        }
    }

    for(int i=0;i<Tb*cfg.Win*cfg.Hin*cfg.Head_x_CHin_div_LTout;i++)   // depth
    {
...
void LN_WEIGHT_AND_BIAS_IN_TRANS_FUNCTION_LN(struct FPGA_HBM_LN_cfg cfg, int *LN_weight, int *LN_bias, int *HBM[])
{
    ...
    int *LN_wt_and_bias_in = (int*)malloc(sizeof(int)*cfg.Head*cfg.CHout);
    if (LN_bias == NULL){perror("main");return;}

    // bit [`MAX_DAT_DW*`L_Tout-1:0] LN_wt_and_bias_mem[2*`CHout_div_LTout];     width = 16*512 = 8192
    int **LN_wt_and_bias_in_mem = (int**)malloc(sizeof(int*)*2*cfg.Head*cfg.CHout_div_LTout);
    for(int i=0;i<2*cfg.Head*cfg.CHout_div_LTout;i++)
    {
    ...
    for(int k=0;k<cfg.Head*cfg.CHout;k++)
        LN_wt_and_bias_in[k] = ((LN_weight[k]<<16)&0xffff0000) + (LN_bias[k]&0x0000ffff);

    for(int i=0;i<cfg.Head;i++)
    {
        for(int j=0;j<cfg.LN_ch_group_times;j++)
        {
            for(int k=0;k<cfg.LN_num_per_AXI_DW;k++)
            {
                if(j*cfg.LN_num_per_AXI_DW+k<cfg.CHin)
                    LN_wt_and_bias_in_mem[i*cfg.LN_ch_group_times+j][k] = LN_wt_and_bias_in[i][j*cfg.LN_num_per_AXI_DW+k];
                else
    ...
    for(int i=0;i<cfg.Head;i++)
    {
        for(int j=0;j<cfg.LN_ch_group_times;j++)
        {
            for(int k=0;k<MAX_DAT_DW*L_Tout/32;k++)
            {
                if(k>=8* 0 & k<8* 1) HBM[ 0][(i*cfg.LN_ch_group_times+j)*HBM_AXI_DATA_WIDTH/32+(k-8* 0)] = LN_wt_and_bias_in_mem[i*cfg.LN_ch_group_times+j][k];
                if(k>=8* 1 & k<8* 2) HBM[ 1][(i*cfg.LN_ch_group_times+j)*HBM_AXI_DATA_WIDTH/32+(k-8* 1)] = LN_wt_and_bias_in_mem[i*cfg.LN_ch_group_times+j][k];
                if(k>=8* 2 & k<8* 3) HBM[ 2][(i*cfg.LN_ch_group_times+j)*HBM_AXI_DATA_WIDTH/32+(k-8* 2)] = LN_wt_and_bias_in_mem[i*cfg.LN_ch_group_times+j][k];
                if(k>=8* 3 & k<8* 4) HBM[ 3][(i*cfg.LN_ch_group_times+j)*HBM_AXI_DATA_WIDTH/32+(k-8* 3)] = LN_wt_and_bias_in_mem[i*cfg.LN_ch_group_times+j][k];
                if(k>=8* 4 & k<8* 5) HBM[ 4][(i*cfg.LN_ch_group_times+j)*HBM_AXI_DATA_WIDTH/32+(k-8* 4)] = LN_wt_and_bias_in_mem[i*cfg.LN_ch_group_times+j][k];
                if(k>=8* 5 & k<8* 6) HBM[ 5][(i*cfg.LN_ch_group_times+j)*HBM_AXI_DATA_WIDTH/32+(k-8* 5)] = LN_wt_and_bias_in_mem[i*cfg.LN_ch_group_times+j][k];
                if(k>=8* 6 & k<8* 7) HBM[ 6][(i*cfg.LN_ch_group_times+j)*HBM_AXI_DATA_WIDTH/32+(k-8* 6)] = LN_wt_and_bias_in_mem[i*cfg.LN_ch_group_times+j][k];
                if(k>=8* 7 & k<8* 8) HBM[ 7][(i*cfg.LN_ch_group_times+j)*HBM_AXI_DATA_WIDTH/32+(k-8* 7)] = LN_wt_and_bias_in_mem[i*cfg.LN_ch_group_times+j][k];
                if(k>=8* 8 & k<8* 9) HBM[ 8][(i*cfg.LN_ch_group_times+j)*HBM_AXI_DATA_WIDTH/32+(k-8* 8)] = LN_wt_and_bias_in_mem[i*cfg.LN_ch_group_times+j][k];
                if(k>=8* 9 & k<8*10) HBM[ 9][(i*cfg.LN_ch_group_times+j)*HBM_AXI_DATA_WIDTH/32+(k-8* 9)] = LN_wt_and_bias_in_mem[i*cfg.LN_ch_group_times+j][k];
                if(k>=8*10 & k<8*11) HBM[10][(i*cfg.LN_ch_group_times+j)*HBM_AXI_DATA_WIDTH/32+(k-8*10)] = LN_wt_and_bias_in_mem[i*cfg.LN_ch_group_times+j][k];
                if(k>=8*11 & k<8*12) HBM[11][(i*cfg.LN_ch_group_times+j)*HBM_AXI_DATA_WIDTH/32+(k-8*11)] = LN_wt_and_bias_in_mem[i*cfg.LN_ch_group_times+j][k];
                if(k>=8*12 & k<8*13) HBM[12][(i*cfg.LN_ch_group_times+j)*HBM_AXI_DATA_WIDTH/32+(k-8*12)] = LN_wt_and_bias_in_mem[i*cfg.LN_ch_group_times+j][k];
                if(k>=8*13 & k<8*14) HBM[13][(i*cfg.LN_ch_group_times+j)*HBM_AXI_DATA_WIDTH/32+(k-8*13)] = LN_wt_and_bias_in_mem[i*cfg.LN_ch_group_times+j][k];
                if(k>=8*14 & k<8*15) HBM[14][(i*cfg.LN_ch_group_times+j)*HBM_AXI_DATA_WIDTH/32+(k-8*14)] = LN_wt_and_bias_in_mem[i*cfg.LN_ch_group_times+j][k];
                if(k>=8*15 & k<8*16) HBM[15][(i*cfg.LN_ch_group_times+j)*HBM_AXI_DATA_WIDTH/32+(k-8*15)] = LN_wt_and_bias_in_mem[i*cfg.LN_ch_group_times+j][k];
                if(k>=8*16 & k<8*17) HBM[16][(i*cfg.LN_ch_group_times+j)*HBM_AXI_DATA_WIDTH/32+(k-8*16)] = LN_wt_and_bias_in_mem[i*cfg.LN_ch_group_times+j][k];
                if(k>=8*17 & k<8*18) HBM[17][(i*cfg.LN_ch_group_times+j)*HBM_AXI_DATA_WIDTH/32+(k-8*17)] = LN_wt_and_bias_in_mem[i*cfg.LN_ch_group_times+j][k];
                if(k>=8*18 & k<8*19) HBM[18][(i*cfg.LN_ch_group_times+j)*HBM_AXI_DATA_WIDTH/32+(k-8*18)] = LN_wt_and_bias_in_mem[i*cfg.LN_ch_group_times+j][k];
                if(k>=8*19 & k<8*20) HBM[19][(i*cfg.LN_ch_group_times+j)*HBM_AXI_DATA_WIDTH/32+(k-8*19)] = LN_wt_and_bias_in_mem[i*cfg.LN_ch_group_times+j][k];
                if(k>=8*20 & k<8*21) HBM[20][(i*cfg.LN_ch_group_times+j)*HBM_AXI_DATA_WIDTH/32+(k-8*20)] = LN_wt_and_bias_in_mem[i*cfg.LN_ch_group_times+j][k];
                if(k>=8*21 & k<8*22) HBM[21][(i*cfg.LN_ch_group_times+j)*HBM_AXI_DATA_WIDTH/32+(k-8*21)] = LN_wt_and_bias_in_mem[i*cfg.LN_ch_group_times+j][k];
                if(k>=8*22 & k<8*23) HBM[22][(i*cfg.LN_ch_group_times+j)*HBM_AXI_DATA_WIDTH/32+(k-8*22)] = LN_wt_and_bias_in_mem[i*cfg.LN_ch_group_times+j][k];
                if(k>=8*23 & k<8*24) HBM[23][(i*cfg.LN_ch_group_times+j)*HBM_AXI_DATA_WIDTH/32+(k-8*23)] = LN_wt_and_bias_in_mem[i*cfg.LN_ch_group_times+j][k];
                if(k>=8*24 & k<8*25) HBM[24][(i*cfg.LN_ch_group_times+j)*HBM_AXI_DATA_WIDTH/32+(k-8*24)] = LN_wt_and_bias_in_mem[i*cfg.LN_ch_group_times+j][k];
                if(k>=8*25 & k<8*26) HBM[25][(i*cfg.LN_ch_group_times+j)*HBM_AXI_DATA_WIDTH/32+(k-8*25)] = LN_wt_and_bias_in_mem[i*cfg.LN_ch_group_times+j][k];
                if(k>=8*26 & k<8*27) HBM[26][(i*cfg.LN_ch_group_times+j)*HBM_AXI_DATA_WIDTH/32+(k-8*26)] = LN_wt_and_bias_in_mem[i*cfg.LN_ch_group_times+j][k];
                if(k>=8*27 & k<8*28) HBM[27][(i*cfg.LN_ch_group_times+j)*HBM_AXI_DATA_WIDTH/32+(k-8*27)] = LN_wt_and_bias_in_mem[i*cfg.LN_ch_group_times+j][k];
                if(k>=8*28 & k<8*29) HBM[28][(i*cfg.LN_ch_group_times+j)*HBM_AXI_DATA_WIDTH/32+(k-8*28)] = LN_wt_and_bias_in_mem[i*cfg.LN_ch_group_times+j][k];
                if(k>=8*29 & k<8*30) HBM[29][(i*cfg.LN_ch_group_times+j)*HBM_AXI_DATA_WIDTH/32+(k-8*29)] = LN_wt_and_bias_in_mem[i*cfg.LN_ch_group_times+j][k];
                if(k>=8*30 & k<8*31) HBM[30][(i*cfg.LN_ch_group_times+j)*HBM_AXI_DATA_WIDTH/32+(k-8*30)] = LN_wt_and_bias_in_mem[i*cfg.LN_ch_group_times+j][k];
                if(k>=8*31 & k<8*32) HBM[31][(i*cfg.LN_ch_group_times+j)*HBM_AXI_DATA_WIDTH/32+(k-8*31)] = LN_wt_and_bias_in_mem[i*cfg.LN_ch_group_times+j][k];  
            }
        }
    }
