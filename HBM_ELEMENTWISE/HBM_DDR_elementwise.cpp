...
#include "General_Map_elementwise.h"

struct FPGA_HBM_ELEMENTWISE_cfg GetFPGA_HBM_ELEMENTWISE_cfg(int Win, int Hin, int CHin, int ElementWise_Mode, uint64_t A_DAT_IN_BASE_ADDR, uint64_t B_DAT_IN_BASE_ADDR, uint64_t DAT_OUT_BASE_ADDR)
{
    struct FPGA_HBM_ELEMENTWISE_cfg ret;
    ...
    ret.Hin                    = Hin;  //5
    ret.CHin                   = CHin; //64
    ret.ElementWise_Mode       = ElementWise_Mode; //2 //0: add, 1: minus, 2: mul
    ...
    ret.A_DAT_IN_BASE_ADDR     = A_DAT_IN_BASE_ADDR;
    ret.DAT_IN_BATCH_STRIDE    =(Pixel_Data_Bytes*Win*Hin*ret.CHin_div_Tout);
    ret.DAT_IN_SURFACE_STRIDE  = (Pixel_Data_Bytes*ret.Win*ret.Hin);
    ret.DAT_IN_LINE_STRIDE     = (Pixel_Data_Bytes*ret.Win);
    
    ret.B_DAT_IN_BASE_ADDR     = B_DAT_IN_BASE_ADDR;
...
void DAT_IN_TRANS_FUNCTION_ELEMENTWISE(struct FPGA_HBM_ELEMENTWISE_cfg cfg, int *dat_in[], int *HBM_DDR[])
{
...
void DAT_OUT_DEMAP_FUNCTION_ELEMENTWISE(struct FPGA_HBM_ELEMENTWISE_cfg cfg, uint16_t *dat_out_mem, uint16_t *dat_out[])
{
